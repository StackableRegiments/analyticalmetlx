...
object QuizResponder {
  def handleResponse(serverName:String,conversationId:String,slideId:String,quizId:String,response:String) ={
    ...
    MeTLXConfiguration.getRoom(conversationId,server.name) ! LocalToServerMeTLStanza(MeTLQuizResponse(Globals.currentUser.is,new Date().getTime,response,Globals.currentUser.is,quizId))
