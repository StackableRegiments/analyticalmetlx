...
class H2Serializer(config:ServerConfiguration) extends Serializer with LiftLogger {
  ...
  def toMeTLSingleChar(i:H2SingleChar):MeTLSingleChar = {
    val cc = decCanvasContent(i)
    MeTLSingleChar(config,cc.author,cc.timestamp,i.char.get,i.x.get,i.y.get,i.width.get,i.height.get,i.fontFamily.get,i.fontSize.get,toColor(i.color.get),i.box.get,cc.identity,cc.target,cc.privacy,cc.slide,cc.audiences)
  }
  override def fromMeTLSingleChar(c:MeTLSingleChar):H2SingleChar = {
    trace("H2Serializer.fromMeTLSingleChar: %s".format(c))
    incCanvasContent(H2SingleChar.create,c,"singleChar").x(c.x).y(c.y).width(c.width).height(c.height).char(c.char).fontFamily(c.fontFamily).fontSize(c.fontSize).color(fromColor(c.color)).box(c.box)
  }
