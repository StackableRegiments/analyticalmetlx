...
class H2Serializer(config:ServerConfiguration) extends Serializer with LiftLogger {
  ...
  override def fromChatMessage(i:MeTLChatMessage):H2ChatMessage = incMeTLContent(H2ChatMessage.create,i,"chatMessage").identity(i.identity).contentType(i.contentType).content(i.content).context(i.context)
