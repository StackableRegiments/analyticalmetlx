...
class PersistedResourceProvider(configName:String,dbInterface:PersistenceInterface,commonBucket:String = "commonBucket"){
  ...
  def insertResource(data:Array[Byte],jid:String = commonBucket):String = {
    dbInterface.insertResource(jid,data)
  }
  def upsertResource(identity:String,data:Array[Byte],jid:String = commonBucket):String = {
    dbInterface.upsertResource(jid,identity,data)
  }
}
