...
case class MeTLVideoStream(override val server:ServerConfiguration,override val author:String,id:String,override val timestamp:Long,url:Box[String],isDeleted:Boolean,override val audiences:List[Audience] = Nil,foreignRelationship:Option[ForeignRelationship]) extends MeTLStanza(server,author,timestamp){
...
object MeTLVideoStream{
  def empty = MeTLVideoStream(ServerConfiguration.empty,"","",0L,Empty,true,Nil,None)
}
