...
// sourced from, with minor modifications: https://github.com/kroky/woot, under MIT opensource permissive license
(function() {
  ...
  this.Woot.QuillAdapter = (function() {
    ...
    function QuillAdapter(io, editor_id, toolbar_id, authors_id,authorObject) {
      ...
      this.io = io;
      this.site_id = siteId !== undefined ? siteId : (Math.floor((Math.random() * 999) + 1)).toString();
      this.color = this.colors[Math.floor(Math.random() * 5)];
      this.editor = new Quill(editor_id, {
        modules: {
          'authorship': {
            authorId: this.site_id,
            enabled: true
          },
          'multi-cursor': {
            timeout: 7000
        ...
        if (op.state && this.site.empty()) {
          ...
          this.editor.updateContents({
            ...
            ops: ops
				...
				// I'm commenting this out, because I'm not yet sure how it's beneficial to start throwing around these emissions with state on them like this - seems like it's just creating chattiness.
				/*
