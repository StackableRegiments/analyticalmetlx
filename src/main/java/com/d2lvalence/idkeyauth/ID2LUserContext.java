...
/**
 * Provides methods to communicate with a server using user credentials and
 * provides some help with handling results and dealing with invalid timestamps
 */
public interface ID2LUserContext {
    ...
    /**
     * Sets the expected time between when the client sends a request and the
     * server receives it
     *
     * @param millis the expected time between when the client sends a request
     * and the server receives it
     */
    public void setServerSkewMillis(long millis);

    /**
     * Returns the expected time between when the client sends a request and the
     * server receives it
     *
     * @return The expected time between when the client sends a request and the
     * server receives it
     */
    public long getServerSkewMillis();

    /**
     * Creates a URI to access the D2L server with the given path and http
     * method
     *
    ...
    /**
     * Calculates the time between when the client sends a request and the
     * server receives it
     *
     ...
     *
     * @param resultCode - http result code, can include the successful result
     * of 2XX in which case the stream is not read
     ...
     * applications will minimally want to check for RESULT_OKAY in order to
     * proceed with interpreting results.
     */
    public int interpretResult(int resultCode, String responseBody);

    /**
     * Returns the parameters used to authenticate with the user in this user
     * context. This allows them to be cached and reloaded.
     *     
     * @return The UserContextParameters which contain information for this
     * ID2LUserContext
     */
    @Deprecated
